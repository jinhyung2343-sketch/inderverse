...
import { getJoinPromptHref, LOGIN_REQUIRED_MESSAGE, sanitizeInternalPath } from '@/lib/guest-policy'
...
export function LoginRequiredAction({
  ...
  const encodedNextPath = encodeURIComponent(sanitizeInternalPath(nextPath, '/main'))
      ...
      {message ? (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-4 text-sm leading-6 text-zinc-200">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <Link
              href="/main"
              className="inline-flex rounded-full border border-white/10 bg-black/20 px-4 py-2 text-xs text-zinc-300 transition hover:bg-white/10"
            >
              허브로 돌아가기
            </Link>
            <Link
              href={`/auth/sign-in?next=${encodedNextPath}`}
